...
"""
Span = number of consecutive days (including today) the price was less than or equal to today's price.
...
Keep a stack of (price, span) pairs.

When a new price comes in:

Pop all previous prices that are ≤ current price.

Add their spans to today’s span.

Push (price, span) back onto the stack.

Return today’s span.

Each price is pushed and popped at most once → O(1) amortized time per query.
"""