...
def removeDuplicate_with_InPlaceSorting(nums):
    
    if not nums:  #Always write edge case
        return 0
    ...
    for i in range(1, len(nums)):
        ...
        # if the i-1 value and i value are same do nothing
...
class Solution(object):
    def removeDuplicates(self, nums):
        ...
        n = len(nums) 
        for num in range(n):
            if not result or result[-1] != nums[num]:
...
print(f"Creating a new array result to remove Duplicate : {abc}")
print(abc)